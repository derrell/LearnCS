/*
 * Assignment: p205 (Malloc'ed array of structs, printed in reverse order)
 *
 ...
 *
 * Open a file called p205-input. Read the first integer from the file, which
 ...
 * separate line, in reverse order (with a single space between the name and
 * the age).
 *
 * Be sure to close any files you open, and to free any memory you allocate.
 *
 * A sample p205-input file might look like this:
 *
 * 6
 * Marlo 17
 * Chris 23
...
int main(int argc, char *argv[])
{
    ...
    // Open the input file
    if ((hFile = fopen("/canning/p205-input", "r")) == NULL)
    {
        fprintf(stderr, "Could not open file p205-input\n");
        return 1;
    }
