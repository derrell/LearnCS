 ...
 */
static char *
o_collect(int c, FILE *stream, char type,
 ...
 */
static char *
f_collect(int c, FILE *stream, unsigned int width)
{
  char *bufp = inp_buf;
  int digit_seen = 0;
...
_doscan(FILE *stream, const char *format, va_list ap)
{
  ...
  while (1)
  {
    ...
    {
    case 'h':
      flags |= FL_SHORT; format++;
      break;
      
    case 'l':
      flags |= FL_LONG; format++;
      break;
      
    case 'L':
      flags |= FL_LONGDOUBLE; format++;
      break;
    }
